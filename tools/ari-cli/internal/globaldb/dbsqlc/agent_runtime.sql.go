// Code generated by sqlc. DO NOT EDIT.
// versions:
//   sqlc v1.31.0
// source: agent_runtime.sql

package dbsqlc

import (
	"context"
)

const appendRunLogMessage = `-- name: AppendRunLogMessage :exec
INSERT INTO run_log_messages (
  message_id,
  workspace_id,
  session_id,
  agent_id,
  sequence,
  role,
  status,
  provider_message_id,
  provider_item_id,
  provider_turn_id,
  provider_response_id,
  provider_call_id,
  provider_channel,
  provider_kind,
  raw_metadata_json
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`

type AppendRunLogMessageParams struct {
	MessageID          string `json:"message_id"`
	WorkspaceID        string `json:"workspace_id"`
	SessionID          string `json:"session_id"`
	AgentID            string `json:"agent_id"`
	Sequence           int64  `json:"sequence"`
	Role               string `json:"role"`
	Status             string `json:"status"`
	ProviderMessageID  string `json:"provider_message_id"`
	ProviderItemID     string `json:"provider_item_id"`
	ProviderTurnID     string `json:"provider_turn_id"`
	ProviderResponseID string `json:"provider_response_id"`
	ProviderCallID     string `json:"provider_call_id"`
	ProviderChannel    string `json:"provider_channel"`
	ProviderKind       string `json:"provider_kind"`
	RawMetadataJson    string `json:"raw_metadata_json"`
}

func (q *Queries) AppendRunLogMessage(ctx context.Context, arg AppendRunLogMessageParams) error {
	_, err := q.db.ExecContext(ctx, appendRunLogMessage,
		arg.MessageID,
		arg.WorkspaceID,
		arg.SessionID,
		arg.AgentID,
		arg.Sequence,
		arg.Role,
		arg.Status,
		arg.ProviderMessageID,
		arg.ProviderItemID,
		arg.ProviderTurnID,
		arg.ProviderResponseID,
		arg.ProviderCallID,
		arg.ProviderChannel,
		arg.ProviderKind,
		arg.RawMetadataJson,
	)
	return err
}

const appendRunLogMessagePart = `-- name: AppendRunLogMessagePart :exec
INSERT INTO run_log_message_parts (
  part_id,
  message_id,
  sequence,
  kind,
  text,
  mime_type,
  uri,
  name,
  tool_name,
  tool_call_id,
  raw_json
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`

type AppendRunLogMessagePartParams struct {
	PartID     string `json:"part_id"`
	MessageID  string `json:"message_id"`
	Sequence   int64  `json:"sequence"`
	Kind       string `json:"kind"`
	Text       string `json:"text"`
	MimeType   string `json:"mime_type"`
	Uri        string `json:"uri"`
	Name       string `json:"name"`
	ToolName   string `json:"tool_name"`
	ToolCallID string `json:"tool_call_id"`
	RawJson    string `json:"raw_json"`
}

func (q *Queries) AppendRunLogMessagePart(ctx context.Context, arg AppendRunLogMessagePartParams) error {
	_, err := q.db.ExecContext(ctx, appendRunLogMessagePart,
		arg.PartID,
		arg.MessageID,
		arg.Sequence,
		arg.Kind,
		arg.Text,
		arg.MimeType,
		arg.Uri,
		arg.Name,
		arg.ToolName,
		arg.ToolCallID,
		arg.RawJson,
	)
	return err
}

const createAgentMessage = `-- name: CreateAgentMessage :exec
INSERT INTO agent_messages (
  agent_message_id,
  workspace_id,
  source_agent_id,
  source_session_id,
  target_agent_id,
  target_session_id,
  body,
  status,
  delivered_session_id
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`

type CreateAgentMessageParams struct {
	AgentMessageID     string `json:"agent_message_id"`
	WorkspaceID        string `json:"workspace_id"`
	SourceAgentID      string `json:"source_agent_id"`
	SourceSessionID    string `json:"source_session_id"`
	TargetAgentID      string `json:"target_agent_id"`
	TargetSessionID    string `json:"target_session_id"`
	Body               string `json:"body"`
	Status             string `json:"status"`
	DeliveredSessionID string `json:"delivered_session_id"`
}

func (q *Queries) CreateAgentMessage(ctx context.Context, arg CreateAgentMessageParams) error {
	_, err := q.db.ExecContext(ctx, createAgentMessage,
		arg.AgentMessageID,
		arg.WorkspaceID,
		arg.SourceAgentID,
		arg.SourceSessionID,
		arg.TargetAgentID,
		arg.TargetSessionID,
		arg.Body,
		arg.Status,
		arg.DeliveredSessionID,
	)
	return err
}

const createAgentMessageContextExcerpt = `-- name: CreateAgentMessageContextExcerpt :exec
INSERT INTO agent_message_context_excerpts (agent_message_id, context_excerpt_id, sequence)
VALUES (?, ?, ?)
`

type CreateAgentMessageContextExcerptParams struct {
	AgentMessageID   string `json:"agent_message_id"`
	ContextExcerptID string `json:"context_excerpt_id"`
	Sequence         int64  `json:"sequence"`
}

func (q *Queries) CreateAgentMessageContextExcerpt(ctx context.Context, arg CreateAgentMessageContextExcerptParams) error {
	_, err := q.db.ExecContext(ctx, createAgentMessageContextExcerpt, arg.AgentMessageID, arg.ContextExcerptID, arg.Sequence)
	return err
}

const createContextExcerpt = `-- name: CreateContextExcerpt :exec
INSERT INTO context_excerpts (
  context_excerpt_id,
  workspace_id,
  source_session_id,
  source_agent_id,
  target_agent_id,
  selector_type,
  selector_json,
  appended_message,
  content_hash
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
`

type CreateContextExcerptParams struct {
	ContextExcerptID string `json:"context_excerpt_id"`
	WorkspaceID      string `json:"workspace_id"`
	SourceSessionID  string `json:"source_session_id"`
	SourceAgentID    string `json:"source_agent_id"`
	TargetAgentID    string `json:"target_agent_id"`
	SelectorType     string `json:"selector_type"`
	SelectorJson     string `json:"selector_json"`
	AppendedMessage  string `json:"appended_message"`
	ContentHash      string `json:"content_hash"`
}

func (q *Queries) CreateContextExcerpt(ctx context.Context, arg CreateContextExcerptParams) error {
	_, err := q.db.ExecContext(ctx, createContextExcerpt,
		arg.ContextExcerptID,
		arg.WorkspaceID,
		arg.SourceSessionID,
		arg.SourceAgentID,
		arg.TargetAgentID,
		arg.SelectorType,
		arg.SelectorJson,
		arg.AppendedMessage,
		arg.ContentHash,
	)
	return err
}

const createContextExcerptItem = `-- name: CreateContextExcerptItem :exec
INSERT INTO context_excerpt_items (
  context_excerpt_id,
  sequence,
  source_message_id,
  source_session_id,
  source_agent_id,
  copied_role,
  copied_text,
  copied_parts_json
) VALUES (?, ?, ?, ?, ?, ?, ?, ?)
`

type CreateContextExcerptItemParams struct {
	ContextExcerptID string `json:"context_excerpt_id"`
	Sequence         int64  `json:"sequence"`
	SourceMessageID  string `json:"source_message_id"`
	SourceSessionID  string `json:"source_session_id"`
	SourceAgentID    string `json:"source_agent_id"`
	CopiedRole       string `json:"copied_role"`
	CopiedText       string `json:"copied_text"`
	CopiedPartsJson  string `json:"copied_parts_json"`
}

func (q *Queries) CreateContextExcerptItem(ctx context.Context, arg CreateContextExcerptItemParams) error {
	_, err := q.db.ExecContext(ctx, createContextExcerptItem,
		arg.ContextExcerptID,
		arg.Sequence,
		arg.SourceMessageID,
		arg.SourceSessionID,
		arg.SourceAgentID,
		arg.CopiedRole,
		arg.CopiedText,
		arg.CopiedPartsJson,
	)
	return err
}

const createHarnessSession = `-- name: CreateHarnessSession :exec
INSERT INTO harness_sessions (
  session_id,
  workspace_id,
  agent_id,
  harness,
  model,
  provider_session_id,
  provider_run_id,
  provider_thread_id,
  cwd,
  folder_scope_json,
  status,
  usage,
  source_session_id,
  source_agent_id,
  prompt_hash,
  context_payload_ids_json,
  permission_mode,
  sandbox_mode,
  tool_scope_json,
  provider_metadata_json
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
`

type CreateHarnessSessionParams struct {
	SessionID             string `json:"session_id"`
	WorkspaceID           string `json:"workspace_id"`
	AgentID               string `json:"agent_id"`
	Harness               string `json:"harness"`
	Model                 string `json:"model"`
	ProviderSessionID     string `json:"provider_session_id"`
	ProviderRunID         string `json:"provider_run_id"`
	ProviderThreadID      string `json:"provider_thread_id"`
	Cwd                   string `json:"cwd"`
	FolderScopeJson       string `json:"folder_scope_json"`
	Status                string `json:"status"`
	Usage                 string `json:"usage"`
	SourceSessionID       string `json:"source_session_id"`
	SourceAgentID         string `json:"source_agent_id"`
	PromptHash            string `json:"prompt_hash"`
	ContextPayloadIdsJson string `json:"context_payload_ids_json"`
	PermissionMode        string `json:"permission_mode"`
	SandboxMode           string `json:"sandbox_mode"`
	ToolScopeJson         string `json:"tool_scope_json"`
	ProviderMetadataJson  string `json:"provider_metadata_json"`
}

func (q *Queries) CreateHarnessSession(ctx context.Context, arg CreateHarnessSessionParams) error {
	_, err := q.db.ExecContext(ctx, createHarnessSession,
		arg.SessionID,
		arg.WorkspaceID,
		arg.AgentID,
		arg.Harness,
		arg.Model,
		arg.ProviderSessionID,
		arg.ProviderRunID,
		arg.ProviderThreadID,
		arg.Cwd,
		arg.FolderScopeJson,
		arg.Status,
		arg.Usage,
		arg.SourceSessionID,
		arg.SourceAgentID,
		arg.PromptHash,
		arg.ContextPayloadIdsJson,
		arg.PermissionMode,
		arg.SandboxMode,
		arg.ToolScopeJson,
		arg.ProviderMetadataJson,
	)
	return err
}

const createHarnessSessionConfig = `-- name: CreateHarnessSessionConfig :exec
INSERT INTO harness_session_configs (agent_id, workspace_id, name, harness, model, prompt)
VALUES (?, ?, ?, ?, ?, ?)
`

type CreateHarnessSessionConfigParams struct {
	AgentID     string  `json:"agent_id"`
	WorkspaceID *string `json:"workspace_id"`
	Name        string  `json:"name"`
	Harness     string  `json:"harness"`
	Model       string  `json:"model"`
	Prompt      string  `json:"prompt"`
}

func (q *Queries) CreateHarnessSessionConfig(ctx context.Context, arg CreateHarnessSessionConfigParams) error {
	_, err := q.db.ExecContext(ctx, createHarnessSessionConfig,
		arg.AgentID,
		arg.WorkspaceID,
		arg.Name,
		arg.Harness,
		arg.Model,
		arg.Prompt,
	)
	return err
}

const deleteHarnessSessionConfig = `-- name: DeleteHarnessSessionConfig :exec
DELETE FROM harness_session_configs
WHERE agent_id = ?
`

type DeleteHarnessSessionConfigParams struct {
	AgentID string `json:"agent_id"`
}

func (q *Queries) DeleteHarnessSessionConfig(ctx context.Context, arg DeleteHarnessSessionConfigParams) error {
	_, err := q.db.ExecContext(ctx, deleteHarnessSessionConfig, arg.AgentID)
	return err
}

const ensureHarnessSessionConfig = `-- name: EnsureHarnessSessionConfig :exec
INSERT OR IGNORE INTO harness_session_configs (agent_id, workspace_id, name, harness, model, prompt)
VALUES (?, ?, ?, ?, ?, ?)
`

type EnsureHarnessSessionConfigParams struct {
	AgentID     string  `json:"agent_id"`
	WorkspaceID *string `json:"workspace_id"`
	Name        string  `json:"name"`
	Harness     string  `json:"harness"`
	Model       string  `json:"model"`
	Prompt      string  `json:"prompt"`
}

func (q *Queries) EnsureHarnessSessionConfig(ctx context.Context, arg EnsureHarnessSessionConfigParams) error {
	_, err := q.db.ExecContext(ctx, ensureHarnessSessionConfig,
		arg.AgentID,
		arg.WorkspaceID,
		arg.Name,
		arg.Harness,
		arg.Model,
		arg.Prompt,
	)
	return err
}

const getContextExcerpt = `-- name: GetContextExcerpt :one
SELECT
  context_excerpt_id,
  workspace_id,
  source_session_id,
  source_agent_id,
  target_agent_id,
  target_session_id,
  selector_type,
  selector_json,
  visibility,
  appended_message,
  content_hash
FROM context_excerpts
WHERE context_excerpt_id = ?
`

type GetContextExcerptParams struct {
	ContextExcerptID string `json:"context_excerpt_id"`
}

type GetContextExcerptRow struct {
	ContextExcerptID string `json:"context_excerpt_id"`
	WorkspaceID      string `json:"workspace_id"`
	SourceSessionID  string `json:"source_session_id"`
	SourceAgentID    string `json:"source_agent_id"`
	TargetAgentID    string `json:"target_agent_id"`
	TargetSessionID  string `json:"target_session_id"`
	SelectorType     string `json:"selector_type"`
	SelectorJson     string `json:"selector_json"`
	Visibility       string `json:"visibility"`
	AppendedMessage  string `json:"appended_message"`
	ContentHash      string `json:"content_hash"`
}

func (q *Queries) GetContextExcerpt(ctx context.Context, arg GetContextExcerptParams) (GetContextExcerptRow, error) {
	row := q.db.QueryRowContext(ctx, getContextExcerpt, arg.ContextExcerptID)
	var i GetContextExcerptRow
	err := row.Scan(
		&i.ContextExcerptID,
		&i.WorkspaceID,
		&i.SourceSessionID,
		&i.SourceAgentID,
		&i.TargetAgentID,
		&i.TargetSessionID,
		&i.SelectorType,
		&i.SelectorJson,
		&i.Visibility,
		&i.AppendedMessage,
		&i.ContentHash,
	)
	return i, err
}

const getHarnessSession = `-- name: GetHarnessSession :one
SELECT session_id, workspace_id, agent_id, harness, model, provider_session_id, provider_run_id, provider_thread_id, cwd, folder_scope_json, status, usage, source_session_id, source_agent_id, prompt_hash, context_payload_ids_json, permission_mode, sandbox_mode, tool_scope_json, provider_metadata_json
FROM harness_sessions
WHERE session_id = ?
`

type GetHarnessSessionParams struct {
	SessionID string `json:"session_id"`
}

type GetHarnessSessionRow struct {
	SessionID             string `json:"session_id"`
	WorkspaceID           string `json:"workspace_id"`
	AgentID               string `json:"agent_id"`
	Harness               string `json:"harness"`
	Model                 string `json:"model"`
	ProviderSessionID     string `json:"provider_session_id"`
	ProviderRunID         string `json:"provider_run_id"`
	ProviderThreadID      string `json:"provider_thread_id"`
	Cwd                   string `json:"cwd"`
	FolderScopeJson       string `json:"folder_scope_json"`
	Status                string `json:"status"`
	Usage                 string `json:"usage"`
	SourceSessionID       string `json:"source_session_id"`
	SourceAgentID         string `json:"source_agent_id"`
	PromptHash            string `json:"prompt_hash"`
	ContextPayloadIdsJson string `json:"context_payload_ids_json"`
	PermissionMode        string `json:"permission_mode"`
	SandboxMode           string `json:"sandbox_mode"`
	ToolScopeJson         string `json:"tool_scope_json"`
	ProviderMetadataJson  string `json:"provider_metadata_json"`
}

func (q *Queries) GetHarnessSession(ctx context.Context, arg GetHarnessSessionParams) (GetHarnessSessionRow, error) {
	row := q.db.QueryRowContext(ctx, getHarnessSession, arg.SessionID)
	var i GetHarnessSessionRow
	err := row.Scan(
		&i.SessionID,
		&i.WorkspaceID,
		&i.AgentID,
		&i.Harness,
		&i.Model,
		&i.ProviderSessionID,
		&i.ProviderRunID,
		&i.ProviderThreadID,
		&i.Cwd,
		&i.FolderScopeJson,
		&i.Status,
		&i.Usage,
		&i.SourceSessionID,
		&i.SourceAgentID,
		&i.PromptHash,
		&i.ContextPayloadIdsJson,
		&i.PermissionMode,
		&i.SandboxMode,
		&i.ToolScopeJson,
		&i.ProviderMetadataJson,
	)
	return i, err
}

const getHarnessSessionConfig = `-- name: GetHarnessSessionConfig :one
SELECT agent_id, workspace_id, name, harness, model, prompt
FROM harness_session_configs
WHERE agent_id = ?
`

type GetHarnessSessionConfigParams struct {
	AgentID string `json:"agent_id"`
}

type GetHarnessSessionConfigRow struct {
	AgentID     string  `json:"agent_id"`
	WorkspaceID *string `json:"workspace_id"`
	Name        string  `json:"name"`
	Harness     string  `json:"harness"`
	Model       string  `json:"model"`
	Prompt      string  `json:"prompt"`
}

func (q *Queries) GetHarnessSessionConfig(ctx context.Context, arg GetHarnessSessionConfigParams) (GetHarnessSessionConfigRow, error) {
	row := q.db.QueryRowContext(ctx, getHarnessSessionConfig, arg.AgentID)
	var i GetHarnessSessionConfigRow
	err := row.Scan(
		&i.AgentID,
		&i.WorkspaceID,
		&i.Name,
		&i.Harness,
		&i.Model,
		&i.Prompt,
	)
	return i, err
}

const getHarnessSessionIdentity = `-- name: GetHarnessSessionIdentity :one
SELECT workspace_id, agent_id
FROM harness_sessions
WHERE session_id = ?
`

type GetHarnessSessionIdentityParams struct {
	SessionID string `json:"session_id"`
}

type GetHarnessSessionIdentityRow struct {
	WorkspaceID string `json:"workspace_id"`
	AgentID     string `json:"agent_id"`
}

func (q *Queries) GetHarnessSessionIdentity(ctx context.Context, arg GetHarnessSessionIdentityParams) (GetHarnessSessionIdentityRow, error) {
	row := q.db.QueryRowContext(ctx, getHarnessSessionIdentity, arg.SessionID)
	var i GetHarnessSessionIdentityRow
	err := row.Scan(&i.WorkspaceID, &i.AgentID)
	return i, err
}

const listAgentMessageContextExcerptIDs = `-- name: ListAgentMessageContextExcerptIDs :many
SELECT context_excerpt_id
FROM agent_message_context_excerpts
WHERE agent_message_id = ?
ORDER BY sequence ASC
`

type ListAgentMessageContextExcerptIDsParams struct {
	AgentMessageID string `json:"agent_message_id"`
}

func (q *Queries) ListAgentMessageContextExcerptIDs(ctx context.Context, arg ListAgentMessageContextExcerptIDsParams) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, listAgentMessageContextExcerptIDs, arg.AgentMessageID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []string{}
	for rows.Next() {
		var context_excerpt_id string
		if err := rows.Scan(&context_excerpt_id); err != nil {
			return nil, err
		}
		items = append(items, context_excerpt_id)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listAgentMessagesByWorkspace = `-- name: ListAgentMessagesByWorkspace :many
SELECT
  agent_message_id,
  workspace_id,
  source_agent_id,
  source_session_id,
  target_agent_id,
  target_session_id,
  body,
  status,
  delivered_session_id
FROM agent_messages
WHERE workspace_id = ?
ORDER BY created_at ASC, agent_message_id ASC
`

type ListAgentMessagesByWorkspaceParams struct {
	WorkspaceID string `json:"workspace_id"`
}

type ListAgentMessagesByWorkspaceRow struct {
	AgentMessageID     string `json:"agent_message_id"`
	WorkspaceID        string `json:"workspace_id"`
	SourceAgentID      string `json:"source_agent_id"`
	SourceSessionID    string `json:"source_session_id"`
	TargetAgentID      string `json:"target_agent_id"`
	TargetSessionID    string `json:"target_session_id"`
	Body               string `json:"body"`
	Status             string `json:"status"`
	DeliveredSessionID string `json:"delivered_session_id"`
}

func (q *Queries) ListAgentMessagesByWorkspace(ctx context.Context, arg ListAgentMessagesByWorkspaceParams) ([]ListAgentMessagesByWorkspaceRow, error) {
	rows, err := q.db.QueryContext(ctx, listAgentMessagesByWorkspace, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListAgentMessagesByWorkspaceRow{}
	for rows.Next() {
		var i ListAgentMessagesByWorkspaceRow
		if err := rows.Scan(
			&i.AgentMessageID,
			&i.WorkspaceID,
			&i.SourceAgentID,
			&i.SourceSessionID,
			&i.TargetAgentID,
			&i.TargetSessionID,
			&i.Body,
			&i.Status,
			&i.DeliveredSessionID,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listContextExcerptItems = `-- name: ListContextExcerptItems :many
SELECT
  sequence,
  source_message_id,
  copied_role,
  copied_text,
  copied_parts_json
FROM context_excerpt_items
WHERE context_excerpt_id = ?
ORDER BY sequence ASC
`

type ListContextExcerptItemsParams struct {
	ContextExcerptID string `json:"context_excerpt_id"`
}

type ListContextExcerptItemsRow struct {
	Sequence        int64  `json:"sequence"`
	SourceMessageID string `json:"source_message_id"`
	CopiedRole      string `json:"copied_role"`
	CopiedText      string `json:"copied_text"`
	CopiedPartsJson string `json:"copied_parts_json"`
}

func (q *Queries) ListContextExcerptItems(ctx context.Context, arg ListContextExcerptItemsParams) ([]ListContextExcerptItemsRow, error) {
	rows, err := q.db.QueryContext(ctx, listContextExcerptItems, arg.ContextExcerptID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListContextExcerptItemsRow{}
	for rows.Next() {
		var i ListContextExcerptItemsRow
		if err := rows.Scan(
			&i.Sequence,
			&i.SourceMessageID,
			&i.CopiedRole,
			&i.CopiedText,
			&i.CopiedPartsJson,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listContextExcerptsByWorkspace = `-- name: ListContextExcerptsByWorkspace :many
SELECT
  context_excerpt_id,
  workspace_id,
  source_session_id,
  source_agent_id,
  target_agent_id,
  target_session_id,
  selector_type,
  selector_json,
  visibility,
  appended_message,
  content_hash
FROM context_excerpts
WHERE workspace_id = ?
ORDER BY created_at ASC, context_excerpt_id ASC
`

type ListContextExcerptsByWorkspaceParams struct {
	WorkspaceID string `json:"workspace_id"`
}

type ListContextExcerptsByWorkspaceRow struct {
	ContextExcerptID string `json:"context_excerpt_id"`
	WorkspaceID      string `json:"workspace_id"`
	SourceSessionID  string `json:"source_session_id"`
	SourceAgentID    string `json:"source_agent_id"`
	TargetAgentID    string `json:"target_agent_id"`
	TargetSessionID  string `json:"target_session_id"`
	SelectorType     string `json:"selector_type"`
	SelectorJson     string `json:"selector_json"`
	Visibility       string `json:"visibility"`
	AppendedMessage  string `json:"appended_message"`
	ContentHash      string `json:"content_hash"`
}

func (q *Queries) ListContextExcerptsByWorkspace(ctx context.Context, arg ListContextExcerptsByWorkspaceParams) ([]ListContextExcerptsByWorkspaceRow, error) {
	rows, err := q.db.QueryContext(ctx, listContextExcerptsByWorkspace, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListContextExcerptsByWorkspaceRow{}
	for rows.Next() {
		var i ListContextExcerptsByWorkspaceRow
		if err := rows.Scan(
			&i.ContextExcerptID,
			&i.WorkspaceID,
			&i.SourceSessionID,
			&i.SourceAgentID,
			&i.TargetAgentID,
			&i.TargetSessionID,
			&i.SelectorType,
			&i.SelectorJson,
			&i.Visibility,
			&i.AppendedMessage,
			&i.ContentHash,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listHarnessSessionConfigs = `-- name: ListHarnessSessionConfigs :many
SELECT agent_id, workspace_id, name, harness, model, prompt
FROM harness_session_configs
WHERE workspace_id = ?
ORDER BY name ASC, agent_id ASC
`

type ListHarnessSessionConfigsParams struct {
	WorkspaceID *string `json:"workspace_id"`
}

type ListHarnessSessionConfigsRow struct {
	AgentID     string  `json:"agent_id"`
	WorkspaceID *string `json:"workspace_id"`
	Name        string  `json:"name"`
	Harness     string  `json:"harness"`
	Model       string  `json:"model"`
	Prompt      string  `json:"prompt"`
}

func (q *Queries) ListHarnessSessionConfigs(ctx context.Context, arg ListHarnessSessionConfigsParams) ([]ListHarnessSessionConfigsRow, error) {
	rows, err := q.db.QueryContext(ctx, listHarnessSessionConfigs, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListHarnessSessionConfigsRow{}
	for rows.Next() {
		var i ListHarnessSessionConfigsRow
		if err := rows.Scan(
			&i.AgentID,
			&i.WorkspaceID,
			&i.Name,
			&i.Harness,
			&i.Model,
			&i.Prompt,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listHarnessSessions = `-- name: ListHarnessSessions :many
SELECT session_id, workspace_id, agent_id, harness, model, provider_session_id, provider_run_id, provider_thread_id, cwd, folder_scope_json, status, usage, source_session_id, source_agent_id, prompt_hash, context_payload_ids_json, permission_mode, sandbox_mode, tool_scope_json, provider_metadata_json
FROM harness_sessions
WHERE workspace_id = ?
ORDER BY created_at ASC, session_id ASC
`

type ListHarnessSessionsParams struct {
	WorkspaceID string `json:"workspace_id"`
}

type ListHarnessSessionsRow struct {
	SessionID             string `json:"session_id"`
	WorkspaceID           string `json:"workspace_id"`
	AgentID               string `json:"agent_id"`
	Harness               string `json:"harness"`
	Model                 string `json:"model"`
	ProviderSessionID     string `json:"provider_session_id"`
	ProviderRunID         string `json:"provider_run_id"`
	ProviderThreadID      string `json:"provider_thread_id"`
	Cwd                   string `json:"cwd"`
	FolderScopeJson       string `json:"folder_scope_json"`
	Status                string `json:"status"`
	Usage                 string `json:"usage"`
	SourceSessionID       string `json:"source_session_id"`
	SourceAgentID         string `json:"source_agent_id"`
	PromptHash            string `json:"prompt_hash"`
	ContextPayloadIdsJson string `json:"context_payload_ids_json"`
	PermissionMode        string `json:"permission_mode"`
	SandboxMode           string `json:"sandbox_mode"`
	ToolScopeJson         string `json:"tool_scope_json"`
	ProviderMetadataJson  string `json:"provider_metadata_json"`
}

func (q *Queries) ListHarnessSessions(ctx context.Context, arg ListHarnessSessionsParams) ([]ListHarnessSessionsRow, error) {
	rows, err := q.db.QueryContext(ctx, listHarnessSessions, arg.WorkspaceID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListHarnessSessionsRow{}
	for rows.Next() {
		var i ListHarnessSessionsRow
		if err := rows.Scan(
			&i.SessionID,
			&i.WorkspaceID,
			&i.AgentID,
			&i.Harness,
			&i.Model,
			&i.ProviderSessionID,
			&i.ProviderRunID,
			&i.ProviderThreadID,
			&i.Cwd,
			&i.FolderScopeJson,
			&i.Status,
			&i.Usage,
			&i.SourceSessionID,
			&i.SourceAgentID,
			&i.PromptHash,
			&i.ContextPayloadIdsJson,
			&i.PermissionMode,
			&i.SandboxMode,
			&i.ToolScopeJson,
			&i.ProviderMetadataJson,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listRunLogMessageParts = `-- name: ListRunLogMessageParts :many
SELECT
  part_id,
  sequence,
  kind,
  text,
  mime_type,
  uri,
  name,
  tool_name,
  tool_call_id,
  raw_json
FROM run_log_message_parts
WHERE message_id = ?
ORDER BY sequence ASC, part_id ASC
`

type ListRunLogMessagePartsParams struct {
	MessageID string `json:"message_id"`
}

type ListRunLogMessagePartsRow struct {
	PartID     string `json:"part_id"`
	Sequence   int64  `json:"sequence"`
	Kind       string `json:"kind"`
	Text       string `json:"text"`
	MimeType   string `json:"mime_type"`
	Uri        string `json:"uri"`
	Name       string `json:"name"`
	ToolName   string `json:"tool_name"`
	ToolCallID string `json:"tool_call_id"`
	RawJson    string `json:"raw_json"`
}

func (q *Queries) ListRunLogMessageParts(ctx context.Context, arg ListRunLogMessagePartsParams) ([]ListRunLogMessagePartsRow, error) {
	rows, err := q.db.QueryContext(ctx, listRunLogMessageParts, arg.MessageID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListRunLogMessagePartsRow{}
	for rows.Next() {
		var i ListRunLogMessagePartsRow
		if err := rows.Scan(
			&i.PartID,
			&i.Sequence,
			&i.Kind,
			&i.Text,
			&i.MimeType,
			&i.Uri,
			&i.Name,
			&i.ToolName,
			&i.ToolCallID,
			&i.RawJson,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const listRunLogMessages = `-- name: ListRunLogMessages :many
SELECT
  message_id,
  workspace_id,
  session_id,
  agent_id,
  sequence,
  role,
  status,
  provider_message_id,
  provider_item_id,
  provider_turn_id,
  provider_response_id,
  provider_call_id,
  provider_channel,
  provider_kind,
  raw_metadata_json
FROM run_log_messages
WHERE session_id = ? AND sequence > ?
ORDER BY sequence ASC, message_id ASC
LIMIT ?
`

type ListRunLogMessagesParams struct {
	SessionID string `json:"session_id"`
	Sequence  int64  `json:"sequence"`
	Limit     int64  `json:"limit"`
}

type ListRunLogMessagesRow struct {
	MessageID          string `json:"message_id"`
	WorkspaceID        string `json:"workspace_id"`
	SessionID          string `json:"session_id"`
	AgentID            string `json:"agent_id"`
	Sequence           int64  `json:"sequence"`
	Role               string `json:"role"`
	Status             string `json:"status"`
	ProviderMessageID  string `json:"provider_message_id"`
	ProviderItemID     string `json:"provider_item_id"`
	ProviderTurnID     string `json:"provider_turn_id"`
	ProviderResponseID string `json:"provider_response_id"`
	ProviderCallID     string `json:"provider_call_id"`
	ProviderChannel    string `json:"provider_channel"`
	ProviderKind       string `json:"provider_kind"`
	RawMetadataJson    string `json:"raw_metadata_json"`
}

func (q *Queries) ListRunLogMessages(ctx context.Context, arg ListRunLogMessagesParams) ([]ListRunLogMessagesRow, error) {
	rows, err := q.db.QueryContext(ctx, listRunLogMessages, arg.SessionID, arg.Sequence, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []ListRunLogMessagesRow{}
	for rows.Next() {
		var i ListRunLogMessagesRow
		if err := rows.Scan(
			&i.MessageID,
			&i.WorkspaceID,
			&i.SessionID,
			&i.AgentID,
			&i.Sequence,
			&i.Role,
			&i.Status,
			&i.ProviderMessageID,
			&i.ProviderItemID,
			&i.ProviderTurnID,
			&i.ProviderResponseID,
			&i.ProviderCallID,
			&i.ProviderChannel,
			&i.ProviderKind,
			&i.RawMetadataJson,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const markRunningHarnessSessionsLost = `-- name: MarkRunningHarnessSessionsLost :exec
UPDATE harness_sessions
SET status = 'lost'
WHERE status = 'running'
`

func (q *Queries) MarkRunningHarnessSessionsLost(ctx context.Context) error {
	_, err := q.db.ExecContext(ctx, markRunningHarnessSessionsLost)
	return err
}

const nextRunLogMessageSequence = `-- name: NextRunLogMessageSequence :one
SELECT CAST(COALESCE(MAX(sequence), 0) + 1 AS INTEGER) AS next_sequence
FROM run_log_messages
WHERE session_id = ?
`

type NextRunLogMessageSequenceParams struct {
	SessionID string `json:"session_id"`
}

func (q *Queries) NextRunLogMessageSequence(ctx context.Context, arg NextRunLogMessageSequenceParams) (int64, error) {
	row := q.db.QueryRowContext(ctx, nextRunLogMessageSequence, arg.SessionID)
	var next_sequence int64
	err := row.Scan(&next_sequence)
	return next_sequence, err
}

const tailRunLogMessages = `-- name: TailRunLogMessages :many
SELECT
  message_id,
  workspace_id,
  session_id,
  agent_id,
  sequence,
  role,
  status,
  provider_message_id,
  provider_item_id,
  provider_turn_id,
  provider_response_id,
  provider_call_id,
  provider_channel,
  provider_kind,
  raw_metadata_json
FROM (
  SELECT
    message_id,
    workspace_id,
    session_id,
    agent_id,
    sequence,
    role,
    status,
    provider_message_id,
    provider_item_id,
    provider_turn_id,
    provider_response_id,
    provider_call_id,
    provider_channel,
    provider_kind,
    raw_metadata_json
  FROM run_log_messages
  WHERE session_id = ?
  ORDER BY sequence DESC, message_id DESC
  LIMIT ?
)
ORDER BY sequence ASC, message_id ASC
`

type TailRunLogMessagesParams struct {
	SessionID string `json:"session_id"`
	Limit     int64  `json:"limit"`
}

type TailRunLogMessagesRow struct {
	MessageID          string `json:"message_id"`
	WorkspaceID        string `json:"workspace_id"`
	SessionID          string `json:"session_id"`
	AgentID            string `json:"agent_id"`
	Sequence           int64  `json:"sequence"`
	Role               string `json:"role"`
	Status             string `json:"status"`
	ProviderMessageID  string `json:"provider_message_id"`
	ProviderItemID     string `json:"provider_item_id"`
	ProviderTurnID     string `json:"provider_turn_id"`
	ProviderResponseID string `json:"provider_response_id"`
	ProviderCallID     string `json:"provider_call_id"`
	ProviderChannel    string `json:"provider_channel"`
	ProviderKind       string `json:"provider_kind"`
	RawMetadataJson    string `json:"raw_metadata_json"`
}

func (q *Queries) TailRunLogMessages(ctx context.Context, arg TailRunLogMessagesParams) ([]TailRunLogMessagesRow, error) {
	rows, err := q.db.QueryContext(ctx, tailRunLogMessages, arg.SessionID, arg.Limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []TailRunLogMessagesRow{}
	for rows.Next() {
		var i TailRunLogMessagesRow
		if err := rows.Scan(
			&i.MessageID,
			&i.WorkspaceID,
			&i.SessionID,
			&i.AgentID,
			&i.Sequence,
			&i.Role,
			&i.Status,
			&i.ProviderMessageID,
			&i.ProviderItemID,
			&i.ProviderTurnID,
			&i.ProviderResponseID,
			&i.ProviderCallID,
			&i.ProviderChannel,
			&i.ProviderKind,
			&i.RawMetadataJson,
		); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
	if err := rows.Close(); err != nil {
		return nil, err
	}
	if err := rows.Err(); err != nil {
		return nil, err
	}
	return items, nil
}

const updateHarnessSessionConfig = `-- name: UpdateHarnessSessionConfig :exec
UPDATE harness_session_configs
SET name = ?, harness = ?, model = ?, prompt = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
WHERE agent_id = ? AND workspace_id = ?
`

type UpdateHarnessSessionConfigParams struct {
	Name        string  `json:"name"`
	Harness     string  `json:"harness"`
	Model       string  `json:"model"`
	Prompt      string  `json:"prompt"`
	AgentID     string  `json:"agent_id"`
	WorkspaceID *string `json:"workspace_id"`
}

func (q *Queries) UpdateHarnessSessionConfig(ctx context.Context, arg UpdateHarnessSessionConfigParams) error {
	_, err := q.db.ExecContext(ctx, updateHarnessSessionConfig,
		arg.Name,
		arg.Harness,
		arg.Model,
		arg.Prompt,
		arg.AgentID,
		arg.WorkspaceID,
	)
	return err
}

const updateHarnessSessionProvider = `-- name: UpdateHarnessSessionProvider :execrows
UPDATE harness_sessions
SET provider_session_id = ?, provider_run_id = ?, provider_metadata_json = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
WHERE session_id = ?
`

type UpdateHarnessSessionProviderParams struct {
	ProviderSessionID    string `json:"provider_session_id"`
	ProviderRunID        string `json:"provider_run_id"`
	ProviderMetadataJson string `json:"provider_metadata_json"`
	SessionID            string `json:"session_id"`
}

func (q *Queries) UpdateHarnessSessionProvider(ctx context.Context, arg UpdateHarnessSessionProviderParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateHarnessSessionProvider,
		arg.ProviderSessionID,
		arg.ProviderRunID,
		arg.ProviderMetadataJson,
		arg.SessionID,
	)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}

const updateHarnessSessionStatus = `-- name: UpdateHarnessSessionStatus :execrows
UPDATE harness_sessions
SET status = ?, updated_at = strftime('%Y-%m-%dT%H:%M:%fZ', 'now')
WHERE session_id = ?
`

type UpdateHarnessSessionStatusParams struct {
	Status    string `json:"status"`
	SessionID string `json:"session_id"`
}

func (q *Queries) UpdateHarnessSessionStatus(ctx context.Context, arg UpdateHarnessSessionStatusParams) (int64, error) {
	result, err := q.db.ExecContext(ctx, updateHarnessSessionStatus, arg.Status, arg.SessionID)
	if err != nil {
		return 0, err
	}
	return result.RowsAffected()
}
