...
func TestPublicOrchestrationMethodInventoryUsesSessionProfileContextNames(t *testing.T) {
	...
	legacyPublic := []string{
		...
		"workspace.deliveries.record_attempt",
		"workspace.deliveries.complete",
		"workspace.deliveries.fail",
...
func TestSessionStartUsesStoredProfileAndSessionTerminology(t *testing.T) {
	...
	created := callMethod[ProfileResponse](t, registry, "profile.create", ProfileCreateRequest{WorkspaceID: "ws-1", Name: "executor", Harness: "test-harness", Model: "model-1", Prompt: "profile behavior", InvocationClass: HarnessInvocationSticky})
	run := callMethod[HarnessSessionStartResponse](t, registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: created.Name, SessionID: "executor-main", Packet: ContextPacket{ID: "ctx_123", WorkspaceID: "ws-1", TaskID: "task-1", PacketHash: "sha256:abc"}, Message: "Start phase 1"})

	if run.Run.HarnessSessionID != "executor-main" || run.Run.SessionID != "executor-main" || run.Run.WorkspaceID != "ws-1" || run.Run.Executor != "test-harness" {
	...
	stored, err := store.GetHarnessSession(context.Background(), "executor-main")
	if err != nil {
...
func TestSessionStartFromProfilePersistsDurableRunLogAndStatusProjection(t *testing.T) {
	...
	run := callMethod[HarnessSessionStartResponse](t, registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: "executor", SessionID: "executor-main", Message: "Start phase 1"})
	if run.Run.HarnessSessionID != "executor-main" || run.Run.Status != "completed" {
...
func TestSessionStartAttachesExistingStoredProfileSessionBeforeHarnessStart(t *testing.T) {
	...
	seedSessionWithPrimaryFolder(t, store, "ws-1", t.TempDir())
	created := callMethod[ProfileResponse](t, registry, "profile.create", ProfileCreateRequest{WorkspaceID: "ws-1", Name: "executor", Harness: "test-harness", Model: "model-1", Prompt: "profile behavior", InvocationClass: HarnessInvocationSticky})
	if err := store.EnsureHarnessSessionConfig(context.Background(), globaldb.HarnessSessionConfig{AgentID: created.ProfileID, WorkspaceID: "ws-1", Name: created.Name, Harness: created.Harness, Model: created.Model, Prompt: created.Prompt}); err != nil {
		t.Fatalf("EnsureHarnessSessionConfig returned error: %v", err)
	}
	if err := store.CreateHarnessSession(context.Background(), globaldb.HarnessSession{SessionID: "executor-main", WorkspaceID: "ws-1", AgentID: created.ProfileID, Harness: "test-harness", Model: "model-1", ProviderSessionID: "provider-existing", Status: "waiting", Usage: "sticky"}); err != nil {
		t.Fatalf("CreateHarnessSession returned error: %v", err)
	}

	run := callMethod[HarnessSessionStartResponse](t, registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: created.Name, SessionID: "executor-main", Message: "Do not launch a duplicate"})
...
func TestSessionStartRejectsExistingSessionFromDifferentWorkspace(t *testing.T) {
	...
	_ = callMethod[ProfileResponse](t, registry, "profile.create", ProfileCreateRequest{WorkspaceID: "ws-1", Name: "executor", Harness: "test-harness", Model: "model-1", Prompt: "profile behavior", InvocationClass: HarnessInvocationSticky})
	created := callMethod[ProfileResponse](t, registry, "profile.create", ProfileCreateRequest{WorkspaceID: "ws-2", Name: "executor", Harness: "test-harness", Model: "model-1", Prompt: "profile behavior", InvocationClass: HarnessInvocationSticky})
	if err := store.EnsureHarnessSessionConfig(context.Background(), globaldb.HarnessSessionConfig{AgentID: created.ProfileID, WorkspaceID: "ws-2", Name: created.Name, Harness: created.Harness, Model: created.Model, Prompt: created.Prompt}); err != nil {
		t.Fatalf("EnsureHarnessSessionConfig returned error: %v", err)
	}
	if err := store.CreateHarnessSession(context.Background(), globaldb.HarnessSession{SessionID: "executor-main", WorkspaceID: "ws-2", AgentID: created.ProfileID, Harness: "test-harness", Model: "model-1", ProviderSessionID: "provider-existing", Status: "waiting", Usage: "sticky"}); err != nil {
		t.Fatalf("CreateHarnessSession returned error: %v", err)
	}

	err := callMethodError(registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: "executor", SessionID: "executor-main", Message: "Do not attach cross-workspace session"})
...
func TestSessionStartRejectsExistingSessionFromDifferentProfile(t *testing.T) {
	...
	seedSessionWithPrimaryFolder(t, store, "ws-1", t.TempDir())
	planner := callMethod[ProfileResponse](t, registry, "profile.create", ProfileCreateRequest{WorkspaceID: "ws-1", Name: "planner", Harness: "test-harness", Model: "model-1", Prompt: "plan", InvocationClass: HarnessInvocationSticky})
	_ = callMethod[ProfileResponse](t, registry, "profile.create", ProfileCreateRequest{WorkspaceID: "ws-1", Name: "executor", Harness: "test-harness", Model: "model-1", Prompt: "execute", InvocationClass: HarnessInvocationSticky})
	if err := store.EnsureHarnessSessionConfig(context.Background(), globaldb.HarnessSessionConfig{AgentID: planner.ProfileID, WorkspaceID: "ws-1", Name: planner.Name, Harness: planner.Harness, Model: planner.Model, Prompt: planner.Prompt}); err != nil {
		t.Fatalf("EnsureHarnessSessionConfig returned error: %v", err)
	}
	if err := store.CreateHarnessSession(context.Background(), globaldb.HarnessSession{SessionID: "shared-session", WorkspaceID: "ws-1", AgentID: planner.ProfileID, Harness: "test-harness", Model: "model-1", ProviderSessionID: "provider-existing", Status: "waiting", Usage: "sticky"}); err != nil {
		t.Fatalf("CreateHarnessSession returned error: %v", err)
	}

	err := callMethodError(registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: "executor", SessionID: "shared-session", Message: "Do not attach wrong profile"})
	data := requireHandlerErrorData(t, err)
...
func TestSessionStartReattachesExistingSessionFromGlobalProfile(t *testing.T) {
	...
	seedSessionWithPrimaryFolder(t, store, "ws-1", t.TempDir())
	globalProfile := globaldb.Profile{ProfileID: "ap_global_executor", Name: "executor", Harness: "test-harness", Model: "model-1", Prompt: "global behavior", InvocationClass: string(HarnessInvocationSticky)}
	if err := store.UpsertProfile(context.Background(), globalProfile); err != nil {
	...
	created := callMethod[HarnessSessionStartResponse](t, registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: "executor", SessionID: "executor-main", Message: "start"})
	if created.Run.HarnessSessionID != "executor-main" || created.Run.WorkspaceID != "ws-1" {
		t.Fatalf("created session = %#v, want executor-main in ws-1", created.Run)
	}
	reattached := callMethod[HarnessSessionStartResponse](t, registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: "executor", SessionID: "executor-main", Message: "reattach"})
	if reattached.Run.HarnessSessionID != "executor-main" || reattached.Run.WorkspaceID != "ws-1" {
...
func TestSessionStartUsesGlobalProfileAcrossWorkspaces(t *testing.T) {
	...
	seedSessionWithPrimaryFolder(t, store, "ws-1", t.TempDir())
	seedSessionWithPrimaryFolder(t, store, "ws-2", t.TempDir())
	if err := store.UpsertProfile(context.Background(), globaldb.Profile{ProfileID: "ap_global_executor", Name: "executor", Harness: "test-harness", Model: "model-1", Prompt: "global behavior", InvocationClass: string(HarnessInvocationSticky)}); err != nil {
		t.Fatalf("UpsertProfile returned error: %v", err)
	}
	d.setHarnessFactoryForTest("test-harness", func(req HarnessSessionStartRequest, primaryFolder string, sink func(string, []TimelineItem)) (HarnessAdapter, error) {
		_ = req
	...
	first := callMethod[HarnessSessionStartResponse](t, registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-1", Profile: "executor", SessionID: "executor-ws-1", Message: "start in ws-1"})
	second := callMethod[HarnessSessionStartResponse](t, registry, "session.start", HarnessSessionStartRequest{WorkspaceID: "ws-2", Profile: "executor", SessionID: "executor-ws-2", Message: "start in ws-2"})
	...
	config, err := store.GetHarnessSessionConfig(context.Background(), "ap_global_executor")
	if err != nil {
...
func TestSessionGetAndListPreservePersistedSessionLinkageFields(t *testing.T) {
	...
	if err := store.CreateHarnessSession(context.Background(), globaldb.HarnessSession{SessionID: "worker-1", WorkspaceID: "ws-1", AgentID: created.ProfileID, Harness: "test-harness", Model: "model-1", Status: "running", Usage: globaldb.HarnessSessionUsageEphemeral, SourceSessionID: "planner-1", SourceAgentID: created.ProfileID}); err != nil {
		t.Fatalf("CreateHarnessSession returned error: %v", err)
	}

	got := callMethod[SessionGetResponse](t, registry, "session.get", SessionGetRequest{SessionID: "worker-1"})
	if got.Session.Usage != globaldb.HarnessSessionUsageEphemeral || got.Session.SourceSessionID != "planner-1" || got.Session.SourceAgentID != created.ProfileID {
		t.Fatalf("session.get = %#v, want persisted usage/source linkage", got.Session)
	}
	listed := callMethod[SessionListResponse](t, registry, "session.list", SessionListRequest{WorkspaceID: "ws-1"})
	if len(listed.Sessions) != 1 || listed.Sessions[0].Usage != globaldb.HarnessSessionUsageEphemeral || listed.Sessions[0].SourceSessionID != "planner-1" || listed.Sessions[0].SourceAgentID != created.ProfileID {
