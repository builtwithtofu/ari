...
func helperWorkspaceContext(workspace *globaldb.Session) string {
	...
	if workspace.OriginRoot != "" {
		return fmt.Sprintf("Scope: workspace %s at %s. Use this workspace context when relevant, and also answer general Ari questions when asked.", workspace.Name, workspace.OriginRoot)
	}
	return fmt.Sprintf("Scope: workspace %s. Use workspace context when relevant, and also answer general Ari questions when asked.", workspace.Name)
}
