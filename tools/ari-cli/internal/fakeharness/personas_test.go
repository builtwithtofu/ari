...
func TestStatefulSessionsTagTurnsAcrossResumes(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "grok named session", harness: "grok", first: []string{"-p", "hi", "-s", "sess-grok", "--output-format", "json"}, resume: []string{"-p", "again", "-s", "sess-grok", "--output-format", "json"}, wantTurn: "(turn 2)"},
	}
...
func TestGrokResumeFlagRequiresExistingSession(t *testing.T) {
	...
	if code, _, _ := runFake(t, "", env, "fake-grok", "-p", "hi", "-s", "sess-1"); code != 0 {
		t.Fatalf("seed session failed")
	}
	code, stdout, _ := runFake(t, "", env, "fake-grok", "-p", "hi", "-r", "sess-1", "--output-format", "json")
...
func TestGrokContinueUsesMostRecentSession(t *testing.T) {
	...
	state := t.TempDir()
	env := []string{EnvHarness + "=grok", EnvStateDir + "=" + state}
	if code, _, _ := runFake(t, "", env, "fake-grok", "-p", "hi", "-s", "sess-old"); code != 0 {
		t.Fatal("seed failed")
	}
	code, stdout, _ := runFake(t, "", env, "fake-grok", "-p", "hi", "-c", "--output-format", "json")
	if code != 0 || !strings.Contains(stdout, `"session_id":"sess-old"`) || !strings.Contains(stdout, `"turn":2`) {
		t.Fatalf("code=%d stdout=%q, want continue of sess-old at turn 2", code, stdout)
	}
}
...
func TestGrokStreamingJSONEmitsEventStream(t *testing.T) {
	...
	for _, want := range []string{`"type":"session_start"`, `"type":"message_delta"`, `"type":"result"`, `"input_tokens":1`} {
		if !strings.Contains(stdout, want) {
			t.Fatalf("stdout %q missing %q", stdout, want)
		}
	}
...
func TestModeListCombinesBehaviorAndStreaming(t *testing.T) {
	...
	if code != 0 || !strings.Contains(stdout, `"type":"result"`) {
