...
// grokPersona answers grok headless invocations: `grok -p ...` with
// --output-format plain|json|streaming-json and -s/-r/-c session flags.
// `grok login` starts OAuth; `grok agent stdio` is routed to the ACP engine.
func grokPersona(run personaRun) int {
	...
	switch format {
	case "json":
		run.out.linef(`{"type":"result","status":"completed","session_id":%q,"result":%q,"usage":{"input_tokens":1,"output_tokens":1},"turn":%d}`, sessionID, text, turn)
	case "streaming-json":
		run.out.linef(`{"type":"session_start","session_id":%q}`, sessionID)
		run.out.linef(`{"type":"message_delta","text":%q}`, text)
		run.out.linef(`{"type":"result","status":"completed","session_id":%q,"usage":{"input_tokens":1,"output_tokens":1},"turn":%d}`, sessionID, turn)
	default:
		run.out.line(text)
	}
	return 0
...
func grokSessionFromArgs(run personaRun) (sessionID string, missing bool) {
	if value, ok := flagValue(run.args, "-s", "--session-id"); ok && strings.TrimSpace(value) != "" {
		// -s creates or resumes a named session.
		return strings.TrimSpace(value), false
	}
...
func partialFailure(run personaRun) int {
	...
	case "grok":
		run.out.line(`{"type":"session_start","session_id":"fake-grok-session"}`)
