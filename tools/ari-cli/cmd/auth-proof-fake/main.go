...
func run() error {
	...
	checks := []struct {
		...
		{name: "doctor", args: []string{"auth", "doctor", "--discover-methods", "--detailed"}, want: []string{"claude", "codex", "opencode"}},
		{name: "status", args: []string{"auth", "status"}, want: []string{"claude", "codex", "opencode", "authenticated"}},
...
func setupProofEnv() (proofEnv, func(), error) {
	...
	fakes := map[string]string{"claude": "fake-claude", "codex": "fake-codex", "opencode": "fake-opencode"}
...
func assertRecordSafe(record string) error {
	...
	for _, want := range []string{"claude", "codex", "opencode", "CODEX_HOME", "CLAUDE_CONFIG_DIR"} {
		if !found[want] {
